...
model = Model()
    ...
    # Objective function
    @objective(model, Max, sum(μ_dddh[d,dd,ddd,h]*profit[d,dd,ddd,h] for d in D, dd in D, ddd in D, h in H))
    ...
    #chance constraint
    @constraint(model,sum(μ_dddh[d,dd,ddd,h] for d in D, dd in D, ddd in D, h in H if profit[d,dd,ddd,h] < 300) <= 0.1)
