...
model = Model()
    ...
    @constraint(model,δ[1,1,1] - δ[1,2,1] <= x[1] )
    @constraint(model,δ[1,2,1] - δ[1,1,1] <= x[1] )
    @constraint(model,δ[1,1,2] - δ[1,2,2] <= x[1] )
    @constraint(model,δ[1,2,2] - δ[1,1,2] <= x[1] )

    @constraint(model,δ[2,1,1] - δ[2,2,1] <= x[2] )
    @constraint(model,δ[2,2,1] - δ[2,1,1] <= x[2] )
    @constraint(model,δ[2,1,2] - δ[2,2,2] <= x[2] )
    @constraint(model,δ[2,2,2] - δ[2,1,2] <= x[2] )

    @constraint(model,δ[3,1,1] - δ[3,2,1] <= x[3] )
    @constraint(model,δ[3,2,1] - δ[3,1,1] <= x[3] )
    @constraint(model,δ[3,1,2] - δ[3,2,2] <= x[3] )
    @constraint(model,δ[3,2,2] - δ[3,1,2] <= x[3] )
